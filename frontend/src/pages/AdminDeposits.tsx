...
import { fetchPendingDeposits, approveDeposit, rejectDeposit } from '../lib/wallet';
...
export default function AdminDeposits() {
  ...
  const load = async () => {
    setLoading(true);
    try {
      const rows = await fetchPendingDeposits();
      setList(rows);
  ...
  const approve = async (id: string) => {
    ...
    try {
      ...
      notify('승인 완료. 해당 업체 잔액에 반영되었습니다.', 'success');
      load();
    } catch (e: any) {
      notify(e.message, 'error');
    } finally {
  ...
  return (
    <div className="max-w-3xl mx-auto p-6">
      <h1 className="text-xl font-bold text-slate-800 mb-4">충전 승인 대기 목록</h1>
      {loading ? <div className="text-slate-400 text-sm">불러오는 중...</div> : (
        <div className="space-y-2">
          {list.length === 0 && <div className="text-slate-400 text-sm">대기중인 충전 신청이 없습니다.</div>}
          {list.map((t) => (
            <div key={t.id} className="bg-white border border-slate-200 rounded-lg p-3 flex items-center justify-between">
              <div>
                <div className="font-medium text-slate-800">{names[t.owner_id] ?? t.owner_id} · {t.amount.toLocaleString('ko-KR')}원 · 입금자 {t.depositor_name || '-'}</div>
